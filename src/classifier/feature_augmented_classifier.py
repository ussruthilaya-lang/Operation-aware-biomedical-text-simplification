...
# Model 3: TF-IDF + UMLS + warning + numerical features (all 8 features)
#
...
def train_model_2():
    ...
    print("\n=== Building features ===")
    # TF-IDF features — max_features=50000 to match the TF-IDF-only baseline
    # exactly, so the comparison isolates the effect of adding domain features
    # rather than confounding it with a smaller TF-IDF vocabulary.
...
def train_model_3():
    ...
    print("\n\n")
    print("=" * 80)
    print("MODEL 3: TF-IDF + All Domain Features (8 features)")
    print("=" * 80)
    ...
    # Feature columns for Model 3 (all 8)
    feature_cols_m3 = [
        "umls_jargon_present",
        "umls_jargon_count",
        "umls_jargon_density",
...
def train_model_2_5():
    """
    Model 2.5: TF-IDF + UMLS + numerical features (6 features, warning dropped).

    Per-feature ablation showed warning_present/warning_count contribute
    ~0 to classification (+0.0008 macro-F1 alone) — expected, since warning
    language tracks output SAFETY, not WHICH operation a sentence needs.
    Keeping them out of the classifier's feature set makes that distinction
    explicit; their real value is in the safety-preservation metric, not here.
    """
    print("\n\n")
    print("=" * 80)
    print("MODEL 2.5: TF-IDF + UMLS + Numerical Features (6 features, warning dropped)")
    print("=" * 80)
    ...
    features_subset = features_df[["pmid", "adaptation", "sent_id"] + feature_cols_m2_5]
    train_merged = train_df.merge(features_subset, on=["pmid", "adaptation", "sent_id"], how="left")
    val_merged = val_df.merge(features_subset, on=["pmid", "adaptation", "sent_id"], how="left")
    ...
    scaler = StandardScaler()
    X_numeric_train = scaler.fit_transform(train_merged[feature_cols_m2_5].fillna(0))
    X_numeric_val = scaler.transform(val_merged[feature_cols_m2_5].fillna(0))
...
if __name__ == "__main__":
    ...
    print(f"Model 2.5 (TF-IDF + UMLS + Numerical):   {m2_5_f1:.3f}  ({m2_5_f1 - 0.424:+.3f})")
    print(f"Model 3 (TF-IDF + all 8):                {m3_f1:.3f}  ({m3_f1 - 0.424:+.3f})")